...
# Build and/or run Corstone examples.
...
OIS_CONFIG="$CHIP_ROOT/config/corstone"
FVP_CONFIG_FILE="$OIS_CONFIG/fvp/cs300.conf"
EXAMPLE_TEST_PATH="$CHIP_ROOT/src/test_driver/corstone/integration-tests"
...
readarray -t SUPPORTED_APP_NAMES <"$CHIP_ROOT"/examples/platform/corstone/supported_examples.txt
SUPPORTED_APP_NAMES+=("unit-tests")

readarray -t TEST_NAMES <"$CHIP_ROOT"/src/test_driver/corstone/unit-tests/test_components.txt

function show_usage() {
    cat <<EOF
Usage: $0 [options] example [test_name]

Build, run or test the Corstone examples and unit-tests.
...
function build_with_cmake() {
    ...
    BUILD_OPTIONS+=(-DCONFIG_CHIP_CORSTONE_SOFTWARE_VERSION="$APP_VERSION")
    ...
    if "$LWIP_DEBUG"; then
        BUILD_OPTIONS+=(-DCONFIG_CHIP_CORSTONE_LWIP_DEBUG=YES)
    fi
...
if [[ "$EXAMPLE" == "unit-tests" ]]; then
    ...
    EXAMPLE_PATH="$CHIP_ROOT/src/test_driver/corstone/unit-tests"
    IS_UNIT_TEST=1
    BUILD_PATH="$BUILD_PATH/unit-tests"
else
    EXAMPLE_PATH="$CHIP_ROOT/examples/$EXAMPLE/corstone"
...
if [[ $IS_UNIT_TEST -eq 0 ]]; then
    EXAMPLE_EXE_PATH="$BUILD_PATH/chip-corstone-$EXAMPLE-example.elf"
