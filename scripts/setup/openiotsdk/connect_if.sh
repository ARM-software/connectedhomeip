...
function connect_with_host {
    ${USER_PREFIX} ip link add name "$HOST_BRIDGE" type bridge
    ${USER_PREFIX} ip link set "$DEFAULT_ROUTE_IF" master "$HOST_BRIDGE"
    ${USER_PREFIX} ip addr flush dev "$DEFAULT_ROUTE_IF"
    for interface in "${INTERFACES[@]}"; do
        ${USER_PREFIX} ip link set "$interface" master "$HOST_BRIDGE"
        ${USER_PREFIX} ip addr flush dev "$interface"
    done
    ${USER_PREFIX} ifconfig "$HOST_BRIDGE" "${default_if_info[ip]}" netmask "${default_if_info[netmask]}" broadcast "${default_if_info[broadcast]}"
    ${USER_PREFIX} route add default gw "${default_if_info[gateway]}" "$HOST_BRIDGE"
}
