...
def write_gn_args(args):
    ...
    for key, value in args.arg_cflags:
        ...
        # Fix " and ' and whitespaces
        filtered_value = map(lambda v: v.replace('"', '\\"'), filtered_value)
        filtered_value = map(lambda v: v.replace("'", '"'), filtered_value)
        filtered_value = map(lambda v: v.strip(), filtered_value)
        # Fix 'imacros' exception
        filtered_value = [v.replace(" ", "") if "imacros" in v else v for v in filtered_value]
        # Fix 'include' exception
        filtered_value = [v.replace(" ", "") if "include" in v else v for v in filtered_value]
        #  Remove duplicates
