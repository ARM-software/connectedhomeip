# Matter Corstone Example Application

These examples run inside an emulated target through the
...
recommended way to build, run and develop with the Corstone port of the Matter
Project. Please read this [VSCode development guide](../VSCODE_DEVELOPMENT.md)
for more information.

Before building the examples, check out the Matter repository and sync Corstone
submodules using the following command:

```
scripts/checkout_submodules.py --shallow --recursive --platform corstone
```
...
the setup easy. In the `scripts/setup/corstone` directory you can find:
...
-   **network_setup.sh** - script to create the specific network namespace and
    ...
    To enable the Corstone networking environment:

    ```
    ${MATTER_ROOT}/scripts/setup/corstone/network_setup.sh up
    ```

    To disable the Corstone networking environment:

    ```
    ${MATTER_ROOT}/scripts/setup/corstone/network_setup.sh down
    ```

    To restart the Corstone networking environment:

    ```
    ${MATTER_ROOT}/scripts/setup/corstone/network_setup.sh restart
    ```
    ...
    Example of the network environment settings:
    ...
    Corstone network setup is also supported via `VScode tasks`:

    -   Open the Command Palette: <kbd>F1</kbd>
    -   Select `Tasks: Run Task`
    -   Select `Setup Corstone network`
...
-   **connect_if.sh** - script that connects specified network interfaces with
    ...
    ```
    ${MATTER_ROOT}/scripts/setup/corstone/connect_if.sh ARMhveth
    ```

    Use `--help` to get more information about the script options.

Corstone network setup scripts contain commands that require root permissions.
Use `sudo` to run the scripts in a user account with root privileges.

After setting up the Corstone network environment the user will be able to run
Matter examples on `FVP` in an isolated network namespace in TAP device mode.
...
> for a running container. This guarantees full isolation of the Corstone
...
-   Docker container environment:
    ...
    -   install the Fast Model Extension package via setup script inside Docker
        ...
        ```
        ${MATTER_ROOT}/scripts/setup/corstone/debugging_setup.sh
        ```
...
`${MATTER_ROOT}/scripts/setup/corstone/python_env_setup.sh`. It installs base
Corstone platform requirements and optional items:
...
-   Matter Python controller - required by integration tests. Use `--controller`
    argument to install.
        ...
        Example:
        ```
        {MATTER_ROOT}/scripts/setup/corstone/python_env_setup.sh --controller
        ```
...
-   `pyedmgr` - required by integration tests. Use `--pyedmgr` argument to
    install.
        ...
        Example:
        ```
        {MATTER_ROOT}/scripts/setup/corstone/python_env_setup.sh --pyedmgr
        ```
...
```
{MATTER_ROOT}/scripts/setup/corstone/python_env_setup.sh --path out/venv
```
...
available
[here](./corstone_platform_overview.md#fvp-persistent-memory-via-files).
...
[Corstone build script](../../scripts/examples/corstone_example.sh) provides the
`-K,--kvsfile` option to use the persistence options listed above.

### Crypto backend

Corstone port supports two crypto backend implementations:

-   `Mbed TLS` - it's the default option
-   [PSA crypto service](https://tf-m-user-guide.trustedfirmware.org/integration_guide/services/tfm_crypto_integration_guide.html)
    from the
    [TrustedFirmware-M (TF-M)](../guides/corstone_platform_overview.md#trusted-firmware-m)
    component
...
`CONFIG_CHIP_CORSTONE_OTA_ENABLE` variable:

```
set(CONFIG_CHIP_CORSTONE_OTA_ENABLE YES)
```
...
by setting `CONFIG_CHIP_CORSTONE_SOFTWARE_VERSION` and
`CONFIG_CHIP_CORSTONE_SOFTWARE_VERSION_STRING` variables.

```
set(CONFIG_CHIP_CORSTONE_SOFTWARE_VERSION "1")
set(CONFIG_CHIP_CORSTONE_SOFTWARE_VERSION_STRING "0.0.1")
```

The default value for `CONFIG_CHIP_CORSTONE_SOFTWARE_VERSION_STRING` is set to
`TFM_NS_APP_VERSION`.
...
```
${MATTER_ROOT}/scripts/examples/corstone_example.sh <example name>
```
...
```
${MATTER_ROOT}/scripts/examples/corstone_example.sh -C run <example name>
```

Run example in specific network namespace with TAP device mode:

```
${MATTER_ROOT}/scripts/run_in_ns.sh ARMns ${MATTER_ROOT}/scripts/examples/corstone_example.sh -C run -n ARMtap <example name>
```
...
`src/test_driver/corstone/integration-tests/<example name>/test_report.json`
file.
...
```
${MATTER_ROOT}/scripts/examples/corstone_example.sh -C test <example name>
```

Testing an example in a specific network namespace with TAP device mode:

```
${MATTER_ROOT}/scripts/run_in_ns.sh ARMns ${MATTER_ROOT}/scripts/examples/corstone_example.sh -C test -n ARMtap <example name>
```
...
2. The example is compiled with debug symbols enabled:
    ...
    ```
    ${MATTER_ROOT}/scripts/examples/corstone_example.sh -d true <example name>
    ```
...
-   Select `Debug Corstone example application` from the drop down list
...
In the case of using the [Corstone network environment](#networking-setup) the
GDB server runs inside a namespace and has the same IP address as the bridge
interface.
...
```
${MATTER_ROOT}/scripts/examples/corstone_example.sh -b psa lock-app
...
```
${MATTER_ROOT}/scripts/examples/corstone_example.sh -b mbedtls lock-app
...
${MATTER_ROOT}/scripts/examples/scripts/run_in_ns.sh ${TEST_NETWORK_NAME}ns
${MATTER_ROOT}/scripts/examples/corstone_example.sh -C test -n ${TEST_NETWORK_NAME}tap
lock-app
```
...
This chapter describes how to add a new Matter example based on Corstone
platform.
...
A new example should be put into `examples/<example_name>/corstone` directory.
It should contain:
...
`chip-corstone-<example_name>-example(_ns)` pattern. The `_ns` suffix is
required for [TF-M applications](#trusted-firmware-m).

Example:

```
set(APP_TARGET chip-corstone-new-example-example_ns)
```

### Integration test

To add the integration test to the new example follow the steps from
[integration tests guide](./corstone_integration_tests.md#add-new-matters-application-test).

### Example tools

Add a new example name to the list in the
`examples/platform/corstone/supported_examples.txt` file. After that the new
example is available in all necessary tools such as helper script
`scripts/examples/corstone_example.sh` or VSCode tasks.
...
`.github/workflows/examples-corstone.yaml` file and add the next step for
`corstone` job step that build this example.
...
- name: Build new-example example
    ...
    run: |
        scripts/examples/corstone_example.sh -b ${{ matrix.cryptoBackend }} new-example
        .environment/pigweed-venv/bin/python3 scripts/tools/memory/gh_sizes.py \
        corstone release new-example \
        examples/new-example/corstone/build/chip-corstone-new-example-example.elf \
        /tmp/bloat_reports/
...
```
...
- name: "Test: new-example example"
    ...
    timeout-minutes: 10
    run: |
        scripts/examples/corstone_example.sh -C test new-example
...
```
