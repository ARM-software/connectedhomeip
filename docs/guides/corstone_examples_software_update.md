# Matter Corstone Example Device Firmware Upgrade

Matter Corstone examples can support over-the-air (`OTA`) Device Firmware
...
Corstone example with
[DFU support](./corstone_examples.md#device-firmware-update) will work as the
...
List of examples that currently support the `DFU` over Matter:
...
4.  Build example application
    ...
    ```
    scripts/examples/corstone_example.sh -v 1 -V 0.0.1 <example_name>
    ```

    This is first version of the application that can be updated.

5.  Build update image

    ```
    scripts/examples/corstone_example.sh -p out/update -v 2 -V 0.0.2 <example_name>
    ```
    ...
    with `.ota` extension (`chip-corstone-<example_name>-example.ota`).

6.  Setup network environment

    ```
    sudo ./scripts/setup/corstone/network_setup.sh -n OISupdate up
    ```

    More details about network environment can be found
    [here](./corstone_examples.md#networking-setup).

7.  In `terminal 1`: run the `OTA Provider` application in the network
    environment and provide a path to the update image:

    ```
    scripts/run_in_ns.sh OISupdate out/ota-provider/chip-ota-provider-app --KVS /tmp/chip-ota-provider --discriminator 3841 --secured-device-port 5580 --filepath out/update/chip-corstone-<example_name>-example.ota
    ```
    ...
    CHIP:SWU: Using OTA file: out/update/chip-corstone-ota-requestor-app-example.ota
    ...
    ```
    ...
    > the example application. `Corstone` examples use the default discriminator
    > value `3840`.
    >
    > The `OTA provider` UDP port must be different from the value used by the
    > example application. `Corstone` examples use the default discriminator
    > value `5540`.

8.  In `terminal 2`: run the example application in the network environment:

    ```
    scripts/run_in_ns.sh OISupdate scripts/examples/corstone_example.sh -C run -n OISupdatetap <example_name>
    ```
...
10. In `terminal 3`: commission the example application into the Matter network:
...
14. Now, you can manually close the nodes applications and terminals sessions:
    ...
    -   Example application - `terminal 2`. More details about terminating
        example can be found [here](./corstone_examples.md#running).

15. Cleanup after update (disable network environment, remove `KVS` storage
    files):

    ```
    sudo ./scripts/setup/corstone/network_setup.sh -n OISupdate down
