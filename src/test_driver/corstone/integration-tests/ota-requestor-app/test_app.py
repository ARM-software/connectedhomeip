...
def binaryPath(request, rootDir):
    if request.config.getoption('binaryPath'):
        ...
        return os.path.join(rootDir, 'examples/ota-requestor-app/corstone/build/chip-corstone-ota-requestor-app-example.elf')
...
def updateBinaryPath(request, rootDir):
    if request.config.getoption('updateBinaryPath'):
        ...
        return os.path.join(rootDir, 'examples/ota-requestor-app/corstone/build/chip-corstone-ota-requestor-app-example.ota')
...
async def test_smoke_test(device):
    ret = await device.wait_for_output("Corstone ota-requestor-app example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Corstone ota-requestor-app example application run")
    assert ret is not None and len(ret) > 0
...
async def test_update_ctrl(device, controller, ota_provider, softwareVersion):
    ...
    ret = await device.wait_for_output("Corstone ota-requestor-app example application start", timeout=1200)
    assert ret is not None and len(ret) > 0
