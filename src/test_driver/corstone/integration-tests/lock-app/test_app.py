...
def binaryPath(request, rootDir):
    if request.config.getoption('binaryPath'):
        ...
        return os.path.join(rootDir, 'examples/lock-app/corstone/build/chip-corstone-lock-app-example.elf')
...
async def test_smoke_test(device):  # noqa: F811
    ret = await device.wait_for_output("Corstone lock-app example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Corstone lock-app example application run")
    assert ret is not None and len(ret) > 0
