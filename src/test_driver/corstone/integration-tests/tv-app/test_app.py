...
def binaryPath(request, rootDir):
    if request.config.getoption('binaryPath'):
        ...
        return os.path.join(rootDir, 'examples/tv-app/corstone/build/chip-corstone-tv-app-example.elf')
...
async def test_smoke_test(device):
    ret = await device.wait_for_output("Corstone tv-app example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Corstone tv-app example application run")
    assert ret is not None and len(ret) > 0
