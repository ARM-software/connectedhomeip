...
def binaryPath(request, rootDir):
    if request.config.getoption('binaryPath'):
        ...
        return os.path.join(rootDir, 'examples/shell/corstone/build/chip-corstone-shell-example.elf')
...
async def test_smoke_test(device):
    ret = await device.wait_for_output("Corstone shell example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Corstone shell example application run")
    assert ret is not None and len(ret) > 0
...
async def test_command_check(device):
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="echo Hello", expected_output="Done")
    assert ret is not None and len(ret) > 1
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="log Hello", expected_output="Done")
    assert ret is not None and len(ret) > 1
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="rand", expected_output="Done")
    assert ret is not None and len(ret) > 1
    ...
    assert ret is not None and len(ret) > 0
    hex_string = "1234"
    ret = await device.send(command="base64 encode {}".format(
        hex_string), expected_output="Done")
    assert ret is not None and len(ret) > 1
    base64code = ret[-2]
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="base64 decode {}".format(
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="version", expected_output="Done")
    assert ret is not None and len(ret) > 1
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="config", expected_output="Done")
    ...
    for param_name, value in config.items():
        ret = await device.wait_for_output("Enter command")
        assert ret is not None and len(ret) > 0
        ret = await device.send(command="config {}".format(
    ...
    ret = await device.send(command="config discriminator {}".format(
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="config discriminator", expected_output="Done")
    assert ret is not None and len(ret) > 1
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="onboardingcodes none", expected_output="Done")
    assert ret is not None and len(ret) > 2
    ...
    for param, value in boarding_codes.items():
        ret = await device.wait_for_output("Enter command")
        assert ret is not None and len(ret) > 0
        ret = await device.send(command="onboardingcodes none {}".format(
    ...
    ret = await device.wait_for_output("Enter command")
    assert ret is not None and len(ret) > 0
    ret = await device.send(command="exit", expected_output="Goodbye")
    assert ret is not None and len(ret) > 0
