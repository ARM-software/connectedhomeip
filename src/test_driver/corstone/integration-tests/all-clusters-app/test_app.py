...
async def test_smoke_test(device):  # noqa: F811
    ret = await device.wait_for_output("Corstone all-clusters-app example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Corstone all-clusters-app example application run")
    assert ret is not None and len(ret) > 0
