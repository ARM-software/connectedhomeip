...
async def test_smoke_test(device):
    ...
    ret = await device.wait_for_output("Open IoT SDK ota-requestor-app example application run", timeout=30)
    assert ret is not None and len(ret) > 0
...
async def test_update_ctrl(device, controller, ota_provider, softwareVersion):
    ...
    ret = await ota_provider.wait_for_output("Commissioning completed successfully", timeout=30)
    assert ret is not None and len(ret) > 0
    ...
    ret = await device.wait_for_output("New version of the software is available", timeout=30)
    ...
    ret = await device.wait_for_output("Current software version", timeout=30)
    assert ret is not None and len(ret) > 1
