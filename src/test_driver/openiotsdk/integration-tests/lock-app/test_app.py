...
def test_lock_ctrl(device, controller):
    ...
    err, res = send_zcl_command(devCtrl, "DoorLock", "SetUser", nodeId, LOCK_CTRL_TEST_ENDPOINT_ID,
                                dict(operationType=DoorLock.Enums.DlDataOperationType.kAdd,
                                     userIndex=LOCK_CTRL_TEST_USER_INDEX,
                                     userName=LOCK_CTRL_TEST_USER_NAME,
                                     userUniqueId=LOCK_CTRL_TEST_USER_INDEX,
                                     userStatus=DoorLock.Enums.DlUserStatus.kOccupiedEnabled,
                                     userType=DoorLock.Enums.DlUserType.kUnrestrictedUser,
                                     credentialRule=DoorLock.Enums.DlCredentialRule.kSingle),
    ...
    err, res = send_zcl_command(devCtrl, "DoorLock", "SetCredential", nodeId, LOCK_CTRL_TEST_ENDPOINT_ID,
                                dict(operationType=DoorLock.Enums.DlDataOperationType.kAdd,
                                     credential=DoorLock.Structs.DlCredential(
                                         credentialType=DoorLock.Enums.DlCredentialType.kPin, credentialIndex=LOCK_CTRL_TEST_CREDENTIAL_INDEX),
    ...
    err, res = send_zcl_command(devCtrl, "DoorLock", "GetCredentialStatus", nodeId, LOCK_CTRL_TEST_ENDPOINT_ID,
                                dict(credential=DoorLock.Structs.DlCredential(
                                    credentialType=DoorLock.Enums.DlCredentialType.kPin, credentialIndex=LOCK_CTRL_TEST_CREDENTIAL_INDEX)),
                                requestTimeoutMs=1000)
    assert err == 0
    ...
    ret = device.wait_for_output("setting door lock state to \"Locked\"".format(LOCK_CTRL_TEST_ENDPOINT_ID))
    assert ret != None and len(ret) > 0
    ...
    ret = device.wait_for_output("setting door lock state to \"Unlocked\"".format(LOCK_CTRL_TEST_ENDPOINT_ID))
    assert ret != None and len(ret) > 0
