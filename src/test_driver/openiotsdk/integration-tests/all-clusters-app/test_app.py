...
from common.fixtures import device, controller  # noqa: F401
...
async def test_smoke_test(device):  # noqa: F811
    ret = await device.wait_for_output("Open IoT SDK all-clusters-app example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Open IoT SDK all-clusters-app example application run", timeout=30)
    assert ret is not None and len(ret) > 0
...
@pytest.mark.asyncio
async def test_clusters_ctrl(device, controller):  # noqa: F811
    assert controller is not None
    devCtrl = controller
    ...
    err, res = await write_zcl_attribute(devCtrl, "AccessControl", "Acl", nodeId,  ALL_CLUSTERS_CTRL_TEST_ENDPOINT_ID,
                                         [entry0, entry1, entry2, entry3])
    assert err == 0
    assert res[0].Status == chip.interaction_model.Status.Success

    err, res = await read_zcl_attribute(devCtrl, "AccessControl", "Acl", nodeId, ALL_CLUSTERS_CTRL_TEST_ENDPOINT_ID)
    assert err == 0
    ...
    err, res = await read_zcl_attribute(devCtrl, "BasicInformation", "VendorID", nodeId, ALL_CLUSTERS_CTRL_TEST_ENDPOINT_ID)
    assert err == 0
    assert res.value == int(setupPayload.attributes['VendorID'])

    #  BasicInformation cluster test - check product ID
    err, res = await read_zcl_attribute(devCtrl, "BasicInformation", "ProductID", nodeId, ALL_CLUSTERS_CTRL_TEST_ENDPOINT_ID)
    assert err == 0
    assert res.value == int(setupPayload.attributes['ProductID'])

    #  BasicInformation cluster test - check software version
    err, res = await read_zcl_attribute(devCtrl, "BasicInformation", "SoftwareVersion", nodeId, ALL_CLUSTERS_CTRL_TEST_ENDPOINT_ID)
    assert err == 0
    assert res.value == int(re.sub(r"[\[\]]", "", version_app[0]))

    #  BasicInformation cluster test - check software version string
    err, res = await read_zcl_attribute(devCtrl, "BasicInformation", "SoftwareVersionString", nodeId, ALL_CLUSTERS_CTRL_TEST_ENDPOINT_ID)
    assert err == 0
