...
from common.utils import connect_device, disconnect_device, discover_device, get_setup_payload, read_zcl_attribute, write_zcl_attribute
...
def test_smoke_test(device):
    ...
    ret = device.wait_for_output("Open IoT SDK all-clusters-app example application run", timeout=30)
    assert ret is not None and len(ret) > 0
...
def test_clusters_ctrl(device, controller):
    ...
    entry0 = acs.AccessControlEntryStruct(privilege=ace.AccessControlEntryPrivilegeEnum.kAdminister, authMode=ace.AccessControlEntryAuthModeEnum.kCase, subjects=NullValue, targets=[acs.Target(cluster=NullValue, endpoint=0, deviceType=NullValue), acs.Target(cluster=1, endpoint=NullValue, deviceType=NullValue), acs.Target(cluster=2, endpoint=3, deviceType=NullValue)], fabricIndex=1)
    entry1 = acs.AccessControlEntryStruct(privilege=ace.AccessControlEntryPrivilegeEnum.kView, authMode=ace.AccessControlEntryAuthModeEnum.kCase, subjects=[4, 5, 6, 7], targets=[acs.Target(cluster=NullValue, endpoint=8, deviceType=NullValue), acs.Target(cluster=9, endpoint=NullValue, deviceType=NullValue), acs.Target(cluster=10, endpoint=11, deviceType=NullValue)], fabricIndex=1)
    entry2 = acs.AccessControlEntryStruct(privilege=ace.AccessControlEntryPrivilegeEnum.kOperate, authMode=ace.AccessControlEntryAuthModeEnum.kGroup, subjects=[12, 13, 14, 15], targets=[acs.Target(cluster=NullValue, endpoint=16, deviceType=NullValue), acs.Target(cluster=17, endpoint=NullValue, deviceType=NullValue), acs.Target(cluster=18, endpoint=19, deviceType=NullValue)], fabricIndex=1)
    entry3 = acs.AccessControlEntryStruct(privilege=ace.AccessControlEntryPrivilegeEnum.kOperate, authMode=ace.AccessControlEntryAuthModeEnum.kCase, subjects=[20, 21, 22, 23], targets=[acs.Target(cluster=NullValue, endpoint=24, deviceType=NullValue), acs.Target(cluster=25, endpoint=NullValue, deviceType=NullValue), acs.Target(cluster=26, endpoint=27, deviceType=NullValue)], fabricIndex=1)
