...
async def test_smoke_test(device):
    ret = await device.wait_for_output("Open IoT SDK shell example application start")
    assert ret is not None and len(ret) > 0
    ret = await device.wait_for_output("Open IoT SDK shell example application run")
    assert ret is not None and len(ret) > 0
