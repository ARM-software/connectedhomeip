...
#include <system/SystemLayerImplOpenIoTSDK.h>
...
CHIP_ERROR LayerImplOpenIoTSDK::EnableSignalSocket()
{
    ...
    // create dymmy signal socket
    int32_t socket_id = iotSocketCreate(IOT_SOCKET_AF_INET6, IOT_SOCKET_SOCK_DGRAM, IOT_SOCKET_IPPROTO_UDP);
    if (socket_id < 0)
    {
    ...
    if (err == CHIP_NO_ERROR)
    {
        ...
        iot_address.s6_addr[15] = 1;

        if (iotSocketBind(socket_id, (uint8_t *) &iot_address, sizeof(iot_address), SIGNAL_SOCKET_PORT) != 0)
        {
...
void LayerImplOpenIoTSDK::Signal()
{
    ...
    else
    {
        ...
        int32_t ret = iotSocketSendTo(mSignalSocket, &dummy, 1, (uint8_t *) &iot_address, sizeof(iot_address), SIGNAL_SOCKET_PORT);
        if (ret < 0)
        {
